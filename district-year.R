...
covariates <- read_dta(file_dy_covariates) %>%
  filter(stateabb %in% c("MA", "MI", "TN")) 

d <- covariates %>% 
  mutate(nces_id = as.numeric(leaidC),
         cohort_year = year) %>%
  inner_join(cells, by=c("nces_id", "year")) %>%
...
std_var <- vars(ind, asn, hsp, blk, wht, frl, nonfrl, rl, nonrl,
                totenrl, nsch, ncharters, speced, ell, tottch, aides,
...
output <- d %>%  
  ...
  mutate_at(std_keep_var, list(std_tot = std)) %>%
  group_by(nces_id) %>%
  ...
  group_by(nces_id) %>%
  arrange(year) %>%
  mutate_at(vars(ends_with("_std")), list(delta = delta)) %>%
  mutate_at(vars(ends_with("_nrm")), list(delta = delta)) %>%
  ungroup() %>%
  select(-ratstutch_whtblk,
         ...
         -flunch_blk,
         -flunch_hsp) %>%
  select(-ind, -asn, -hsp, -blk, -wht, -frl, -nonfrl, -rl, -nonrl,
         -totenrl, -nsch, -ncharters, -speced, -ell, -tottch, -aides,
         -corsup, 
         -stutch_wht,
         -stutch_blk,
         ...
         -incVmalfem,
         -educVmalfem) %>%
  arrange(nces_id, year) %>%
  select(-year)

# hack-y bullshit
output[is.na(output)] <- 0

write_csv(output, file_delta_std_dm_cleaned)
...
output %>%
  ...
  write_csv(file_delta_std_dm_train)

output %>%
  filter(nces_id %in% districts[val_dist]) %>%
  write_csv(file_delta_std_dm_val)

output %>%
  filter(nces_id %in% districts[test_dist]) %>%
  write_csv(file_delta_std_dm_test)
