...
coh_std <- function(x, na.rm = TRUE) abs((x - mean(x, na.rm = na.rm)) / sd(x, na.rm))
...
output <- d %>%  
  select(-leaidC, -leaname, -fips, -stateabb, -subject, 
         -starts_with("metro"), -starts_with("micro"),
          -czid, -starts_with("county"), -czid, -cdcode, -ends_with("fleslope"), 
          -gslo, -gshi, -elmtch) %>%
  mutate_at(nrm_var, list(nrm_tot = nrm)) %>%
  mutate_at(std_var, list(std_tot = std)) %>%
  mutate_at(std_keep_var, list(std_tot = std)) %>%
  group_by(nces_id, cohort) %>%
  mutate_at(nrm_var, list(nrm_coh = nrm)) %>%
  mutate_at(std_var, list(std_coh = std)) %>%
  mutate_at(std_keep_var, list(std_coh = std)) %>%
  ungroup() %>%
  ...
  select(-ind, -asn, -hsp, -blk, -wht, -frl, -nonfrl, -rl, -nonrl, -frlunch,
         ...
         -incVmalfem,
         -educVmalfem,
         -ends_with("nrm_tot_nrm_coh"),
         -ends_with("std_tot_st")) %>%
...
write_csv(output, file_level_std_cleaned)
...
output %>%
  ...
  write_csv(file_level_std_train)

output %>%
  filter(nces_id %in% districts[val_dist]) %>%
  write_csv(file_level_std_val)

output %>%
  filter(nces_id %in% districts[test_dist]) %>%
  write_csv(file_level_std_test)
